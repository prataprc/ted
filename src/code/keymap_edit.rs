    ...
    fn normal_fold(&mut self, evnt: Event) -> Result<Event> {
        ...
        let (prefix, evnt) = match prefix {
            ...
            G(n) if m_empty => match evnt {
                ...
                End(_m) => (noop, Mt(Mto::ScreenEnd(n + 1, DP::None))),
                Char('_', _m) => (noop, Mt(Mto::LineEnd(n + 1, DP::TextCol))),
                Char('0', _m) => (noop, Mt(Mto::ScreenHome(DP::None))),
                Char('^', _m) => (noop, Mt(Mto::ScreenHome(DP::TextCol))),
                Char('$', _m) => (noop, Mt(Mto::ScreenEnd(n + 1, DP::None))),
                Char('m', _m) => (noop, Mt(Mto::ScreenMiddle)),
                Char('M', _m) => (noop, Mt(Mto::LineMiddle(n + 1, DP::None))),
                // motion command - linewise
                Char('k', _m) => (noop, Mt(Mto::ScreenUp(n + 1, DP::None))),
                Up(_m) => (noop, Mt(Mto::ScreenUp(n + 1, DP::None))),
                Char('j', _m) => (noop, Mt(Mto::ScreenDown(n + 1, DP::None))),
                Down(_m) => (noop, Mt(Mto::ScreenDown(n + 1, DP::None))),
                Char('g', _) => (noop, Mt(Mto::Row(n, DP::TextCol))),
                // motion command - wordwise
